...
class Reditn {
	...
	public static function getLinkType(ourl:String, cb:LinkType -> Void):Void {
		...
		if(url.startsWith("reddit.com/r/") && url.indexOf("/comments/") != -1)
			...
			/") || ((url.indexOf(".wordpress.com/") != -1 && url.indexOf("files.wordpress.com") == -1) && url.lastIndexOf("/") != url.indexOf("/")) || (url.indexOf(".blogger.com/") != -1 && url.lastIndexOf("/") != url.indexOf("/")) || (url.indexOf(".blogspot.") != -1 && url.lastIndexOf("/") != url.indexOf("/")))
			cb(LinkType.ARTICLE);
