...
// ==/UserScript==
(function() {
	var settings = localStorage.reditn ? JSON.parse(localStorage.reditn) : {
	};
	var offset=3;
	...
	function image_url(ourl, el, cb) {
		...
		if(url.substr(0, 12) == "i.imgur.com/" && url.split(".").length == 3)
			...
			jQuery.getJSON("http://backend.deviantart.com/oembed?url="+encodeURI(url)+"&format=jsonp&callback=?",
			function(data){
				console.log(data);
				cb(data.url, el);
			});
	...
	function preload(url) {
		...
		link.href = url;
		link.rel = navigator.userAgent.toLowerCase().indexOf('chrome') > -1 ? "prefetch" : "preload";
		document.head.appendChild(link);
	}
	var maxw = (window.innerWidth - document.body.getElementsByClassName("tagline")[0].offsetLeft) - document.body.getElementsByClassName("side")[0].offsetWidth;
	...
	function on_mouse_over_user(e) {
		...
		jQuery.getJSON("/user/"+user+"/about.json", {}, function(d){
			d = d.data;
	...
	function on_mouse_over_reddit(e) {
		...
		var div = document.createElement("div");
		pop_up(this, div, this.offsetLeft + this.offsetWidth + offset, this.offsetTop);
		jQuery.getJSON("/r/"+reddit+"/about.json", {}, function(d) {
			d = d.data;
